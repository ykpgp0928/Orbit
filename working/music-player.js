 ...
 * FWF working — Phase 1–4
 *   Phase 1 shellSync · Phase 2 Template · Phase 3 Interaction · Phase 4 Music
 * =============================================================================
 * Interaction: Gesture · Drag · Snap · Dock · Layout  (src/interaction/*)
 * Music Widget: AudioEngine · PlaylistSource · MusicWidget  (src/media, src/widgets/music)
 * Runtime does not own tracks; playback goes through AudioEngine surface.
 * =============================================================================
 */
(function () {
  "use strict";
  ...
  function createSnap(config, ctx) {
    var magnetSideLocal = null;

    function getBallSizeInner() {
      return ctx.isMobile() ? config.ballSizeMobile : config.ballSize;
    }

    function getSnapTargetsInner() {
      var rootEl = ctx.getRoot();
      var vw = window.innerWidth;
      var leftBase = rootEl ? parseFloat(getComputedStyle(rootEl).left) || 20 : 20;
      var w = getBallSizeInner();
      var edgePad = ctx.isMobile() ? 12 : 16;
      return {
        leftBase: leftBase,
        w: w,
    ...
    function getSnapDistancesInner() {
      var mobile = ctx.isMobile() || "ontouchstart" in window;
      return {
    ...
    function applyMagneticXInner(freeX, clientX, session) {
      var t = getSnapTargetsInner();
      var d = getSnapDistancesInner();
      var absLeft = t.leftBase + freeX;
      var absRight = absLeft + t.w;
      session = session || {};
      ...
      function setMagnet(side) {
        magnetSideLocal = side;
        if (ctx.onMagnetChange) ctx.onMagnetChange(side);
      }

      if (magnetSideLocal === "left") {
        var tentativeL = getOx() + (clientX - getStartX());
        if (tentativeL < t.leftX) {
          setOx(t.leftX - (clientX - getStartX()));
          return t.leftX;
        }
        if (tentativeL - t.leftX >= d.leave) {
          setMagnet(null);
          return tentativeL;
        }
        return t.leftX;
      }
      if (magnetSideLocal === "right") {
        var tentativeR = getOx() + (clientX - getStartX());
        if (tentativeR > t.rightX) {
          setOx(t.rightX - (clientX - getStartX()));
          return t.rightX;
        }
        if (t.rightX - tentativeR >= d.leave) {
          setMagnet(null);
          return tentativeR;
        }
        return t.rightX;
    ...
    function snapToEdgeInner() {
      var t = getSnapTargetsInner();
      var d = getSnapDistancesInner();
      var px = ctx.getPosX();
      var py = ctx.getPosY();
      var absLeft = t.leftBase + px;
      var absRight = absLeft + t.w;
      var targetX = px;
      var finalSide = null;

      if (magnetSideLocal === "left" || absLeft < d.enter) {
        targetX = t.leftX;
        finalSide = "left";
      } else if (magnetSideLocal === "right" || absRight > t.vw - d.enter) {
        targetX = t.rightX;
        finalSide = "right";
      }

      var pair = ctx.clampPosition(targetX, py);
      var moved = Math.abs(pair[0] - px) > 0.5 || Math.abs(pair[1] - py) > 0.5;
      ctx.setPosition(pair[0], pair[1]);
      magnetSideLocal = null;
    ...
    function isNearDockEdgeInner(px) {
      var x = px != null ? px : ctx.getPosX();
      var t = getSnapTargetsInner();
      var d = getSnapDistancesInner();
      var absLeft = t.leftBase + x;
      var absRight = absLeft + t.w;
      var th = d.enter + 20;
      if (absLeft < th || Math.abs(x - t.leftX) < 10) return "left";
      if (absRight > t.vw - th || Math.abs(x - t.rightX) < 10) return "right";
      return null;
    }

    function syncDockFromPositionInner(px, isDragging) {
      ...
      var absLeft = t.leftBase + x;
      var absRight = absLeft + t.w;
      var threshold = d.enter + 8;
      if (absLeft < threshold || Math.abs(x - t.leftX) < 4) return "left";
      if (absRight > t.vw - threshold || Math.abs(x - t.rightX) < 4) return "right";
      return null;
    }

    return {
      applyMagneticX: applyMagneticXInner,
      snapToEdge: snapToEdgeInner,
      clearMagnet: function () {
        magnetSideLocal = null;
        if (ctx.onMagnetChange) ctx.onMagnetChange(null);
      },
      getMagnetSide: function () {
        return magnetSideLocal;
      },
      isNearDockEdge: isNearDockEdgeInner,
      syncDockFromPosition: syncDockFromPositionInner,
      getSnapTargets: getSnapTargetsInner,
      getSnapDistances: getSnapDistancesInner,
      getBallSize: getBallSizeInner,
    };
  }
  ...
  function createDock(handlers) {
    var h = handlers || {};
    var state = { side: null, expanded: false, closing: false };
    var closeTimer = null;

    function getState() {
      return { side: state.side, expanded: state.expanded, closing: state.closing };
    }
    function isDocked() {
      return !!state.side;
    }
  ...
  function createLayout(ctx) {
    ...
    function updateExpandDirectionInner() {
      ...
      var rootEl = ctx.getRoot();
      var leftBase = parseFloat(getComputedStyle(rootEl).left) || 20;
      var vw = window.innerWidth;
      var absLeft = leftBase + ctx.getPosX();
      var openW = 360;
      try {
        var raw = getComputedStyle(rootEl).getPropertyValue("--mp-width-open").trim();
        if (raw) {
      ...
      setExpandLeft(absLeft + openW > vw - 12);
    }

    function getMobileCardSizeInner() {
      var vw = window.innerWidth;
      var vh = window.innerHeight;
      var w = Math.min(vw * 0.72, 260);
      var h = Math.min(vh * 0.34, 210);
      return { w: Math.max(180, Math.round(w)), h: Math.max(140, Math.round(h)) };
    }

    function prepareMobileOpenInner() {
      if (!ctx.getRoot() || !ctx.isMobile()) return;
      if (ctx.isDocked()) return;

      var size = getMobileCardSizeInner();
      var openW = size.w;
      var openH = size.h;
      var rootEl = ctx.getRoot();
      var leftBase = parseFloat(getComputedStyle(rootEl).left) || 20;
      var ballW = ctx.getBallSize();
      var vw = window.innerWidth;
      var pad = 8;
      var absLeft = leftBase + ctx.getPosX();
      var el = false;

      if (absLeft + openW <= vw - pad) el = false;
      else if (absLeft + ballW - openW >= pad) el = true;
      else {
        el = true;
        var minAbs = pad - ballW + openW;
        var maxAbs = vw - pad - ballW;
        absLeft = clamp(
      ...
      var pair = ctx.clampPosition(ctx.getPosX(), ctx.getPosY(), ballW, openH);
      ctx.setPosition(pair[0], pair[1]);
      setExpandLeft(el);
    }

    function updateListDirectionInner() {
      var rootEl = ctx.getRoot();
      if (!rootEl) return;
      var rect = rootEl.getBoundingClientRect();
      var listH =
        parseInt(getComputedStyle(rootEl).getPropertyValue("--mp-list-h"), 10) ||
        280;
    ...
    function shouldDockDown() {
      var rootEl = ctx.getRoot();
      if (!rootEl || !ctx.isMobile() || !ctx.isDocked()) return false;
      var rect = rootEl.getBoundingClientRect();
      var n = rootEl.querySelectorAll(".mp-dock-btn").length || 5;
      var stackH = n * 40 + (n - 1) * 8 + 12;
      return (
        rect.top < stackH + 8 && window.innerHeight - rect.bottom > rect.top
      );
    }

    return {
      getExpandLeft: function () {
        return expandLeft;
      },
      setExpandLeft: setExpandLeft,
      updateExpandDirection: updateExpandDirectionInner,
      prepareMobileOpen: prepareMobileOpenInner,
      getMobileCardSize: getMobileCardSizeInner,
      updateListDirection: updateListDirectionInner,
      getListUp: function () {
        return listUp;
      },
      setListUp: function (v) {
        listUp = !!v;
        if (ctx.onListUp) ctx.onListUp(listUp);
        if (ctx.sync) ctx.sync();
      },
      shouldDockDown: shouldDockDown,
    };
  }
  ...
  function createAudioEngine(handlers) {
    ...
    function ensure() {
      ...
      el.preload = "metadata";
      el.volume = vol;
      el.addEventListener("timeupdate", function () { if (h.onTimeUpdate) h.onTimeUpdate(); });
      el.addEventListener("ended", function () { if (h.onEnded) h.onEnded(); });
      el.addEventListener("play", function () { if (h.onPlay) h.onPlay(); });
      el.addEventListener("pause", function () { if (h.onPause) h.onPause(); });
      el.addEventListener("error", function () { if (h.onError) h.onError(); });
      el.addEventListener("loadedmetadata", function () { if (h.onLoadedMetadata) h.onLoadedMetadata(); });
      return el;
    }
    return {
      ensure: ensure,
      setSource: function (url) { ensure(); el.src = url || ""; el.load(); },
      play: function () { ensure(); var p = el.play(); if (p && p.catch) p.catch(function () {}); return p; },
      pause: function () { if (el) el.pause(); },
      toggle: function () { ensure(); if (el.paused) return this.play(); this.pause(); },
      seek: function (ratio) {
        if (!el || !isFinite(el.duration)) return;
        el.currentTime = Math.min(1, Math.max(0, ratio)) * el.duration;
      },
      setCurrentTime: function (t) { if (el) el.currentTime = t; },
      setVolume: function (v) { vol = Math.min(1, Math.max(0, v)); if (el) el.volume = vol; },
      getVolume: function () { return vol; },
      getCurrentTime: function () { return el ? el.currentTime || 0 : 0; },
      getDuration: function () { return el && isFinite(el.duration) ? el.duration : 0; },
      isPaused: function () { return !el || el.paused; },
      hasSource: function () { return !!(el && el.src); },
  ...
  function createGesture(config, handlers) {
    ...
    function onPointerMove(e) {
      ...
      if (!gDragging && moveDist > config.clickThreshold) {
        clearLongPress();
        gDragging = true;
        longPressTriggered = false;
        if (handlers.onDragStart) handlers.onDragStart(e);
      ...
      return gDragging ? "drag" : "pending";
    }

    function onPointerUp(e, session) {
      var pid = session && session.pointerId != null ? session.pointerId : e && e.pointerId;
      if (
        session &&
      ...
      clearLongPress();
      activePointer = null;
      if (session && session.wasDragging) return;
    ...
    return {
      ...
      getActivePointer: function () { return activePointer; },
      getStart: function () { return { x: startClientX, y: startClientY }; },
      isDragging: function () { return gDragging; },
      wasLongPress: function () { return longPressTriggered; },
      getMoveDist: function () { return moveDist; },
      getIgnoreUntil: function () { return ignoreUntil; },
    };
  }
  ...
  function createDrag(ctx) {
    ...
    function move(dx, dy, clientX, pointerStart) {
      ...
      var pair = ctx.clampPosition(freeX, originY_ + dy);
      ctx.setPosition(pair[0], pair[1]);
    }
